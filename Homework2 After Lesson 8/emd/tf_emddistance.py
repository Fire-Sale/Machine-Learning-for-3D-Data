...
emd_distance_module=tf.load_op_library('./tf_emddistance_so.so')

def emd_distance(xyz1,xyz2):
	'''
...
output: idx2:  (batch_size,#point)   bipartite mapping from second to first
	'''
	print ('emd')
	return emd_distance_module.emd_distance(xyz1,xyz2)
#@tf.RegisterShape('emddistance')
#def _emd_distance_shape(op):
...
@ops.RegisterGradient('EmdDistance')
def _emd_distance_grad(op,grad_dist1,grad_idx1,grad_idx2):
	xyz1=op.inputs[0]
	xyz2=op.inputs[1]
	idx1=op.outputs[1]
	idx2=op.outputs[2]
	return emd_distance_module.emd_distance_grad(xyz1,xyz2,grad_dist1,idx1,idx2)
...
if __name__=='__main__':
	...
	with tf.Session('') as sess:
		...
		if True:
			...
			reta,retb,retc,retd=emd_distance(inp1,inp2)
		...
		for i in xrange(100):
			...
			print (str(i)+","+str(trainloss) + "," +str((newt-t0)/(i+1)) + "," + str(best))
			t1=newt
